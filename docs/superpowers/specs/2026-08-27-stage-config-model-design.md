...
- **Escape hatch**: `NETHACKERS_STAGE_FILE=/path/to/file` short-circuits the walk to an explicit file; `NETHACKERS_STAGE_FILE=` (empty) ignores any file — pure defaults + env, e.g. for hitting prod from inside a worktree.
...
Two phases; every PR ships and reverts independently. Rollback at any step is deleting a file or reverting one module — no step rewrites the hub, the deploy path, or prod compose.

**Phase 1 — the foundation (PRs 1–4):**

1. **PR 1 — `config.py`, zero behavior change.** `Stage` + `load_stage()` (defaults + env layer only; no file reading yet) + `DEV_TOKEN`/`DEV_OWNER`. Repoints every literal named in the entrypoint table: `cli.py:230` (deleting `_default_hub`, `:108-109`), `:269,281,308,321,331,405,414,544-545`; `launch.py:51-52,62-66,71,83,185`; `home.py:34`; `runs.py:21`; `evolve_form.py:48,326-327`; `register.py:33-34,62`. Fixes the stale help (`cli.py:197-198`). Golden test pins `Stage()` == today's literals. Kills K1, K3 (client), K5, K6, K7 **as drift** immediately.
2. **PR 2 — the worktree stack.** `scripts/stack.py`, the `stack` make target, recipe-level `set -a; . ./.env.stack` sourcing, the `wait-hub`/echo port fix (`Makefile:56,65,68`), `.gitignore` += `.env.stack`, `.nethackers/`. Generated keys: `NETHACKERS_STAGE`, `NETHACKERS_HUB`, `NETHACKERS_HUB_PORT`, `COMPOSE_PROJECT_NAME`, `NETHACKERS_DATA_ROOT`. Python doesn't read the file yet, but `make up` per worktree already yields an isolated port + project + volume. Kills K2 and surfaces 1–2.
3. **PR 3 — the loader reads the file.** `_find_stack_file` walk-up + `NETHACKERS_STAGE_FILE` hatch + the two indicators (CLI stderr line in `_run`; idbar at `app.py:95,205`). Now every command inside a worktree auto-targets its stack and its `data_root`. Kills surface 3 and the "evolve pointed at prod with dev-token" bug class.
4. **PR 4 — throwaway names.** Generator additionally writes `NETHACKERS_REPO_NAME=nh-dev-<slug>`, `NETHACKERS_ARENA_IMAGE=nethackers/arena:<slug>`, `NETHACKERS_MUTATOR_IMAGE=nethackers/mutator:latest`; Makefile gains `-include .env.stack` + the `$(or ...)` tag defaults. Kills surfaces 4–5 (the arena suffix is cheap — only the `src/` COPY layers above the shared `nle-base` are per-worktree).

**Phase 2 — local-run enablement (PR 5):**

5. **PR 5 — real-auth hub + the recipe.** Split `compose.yaml` into base + `compose.override.yaml` (stub, today's env verbatim) + `compose.github.yaml`; `HUB_AUTH` make switch; the two drift tests (stub map == `DEV_TOKEN`/`DEV_OWNER`; github overlay's default client id == `Stage().github_client_id`); `docs/local-stack.md` carrying the end-to-end recipe, the validation checklist, and the self-hosting section. After this PR the Validation section is a runnable session.

## Testing

- **Golden stage test**: `Stage()` equals today's literals field by field (`hub_url == "https://nethackers.dunnolab.ai"`, `arena_image == "nethackers/arena:dev"`, …) — the PR 1 no-behavior-change proof, and the permanent pin that prod is the defaults.
...
- **Walk-up discovery test**: found from a nested cwd; `NETHACKERS_STAGE_FILE=/x` overrides the walk; `NETHACKERS_STAGE_FILE=` (empty) ignores an existing file.
...
- **No per-worktree GitHub automation beyond the name default.** `nh-dev-<slug>` is one generated line; publish stays correctly disabled for the anonymous `dev` owner (`launch.py:94`).

## Open risks

1. **Implicit discovery is action-at-a-distance.** The same command answers differently by `$PWD`. Mitigated by the required indicator (stderr line + idbar) — git's own model: cwd picks the config, the prompt shows the branch. If that proves insufficient in practice, the designed fallback is repo-scoped discovery: `_find_stack_file` accepts only a `.env.stack` sitting next to a `.git` entry — a one-function tightening, no schema change.
