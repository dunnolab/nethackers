...
3. **PR 3 — the loader reads the file.** `_find_stack_file` walk-up + the `NETHACKERS_STAGE_FILE` and `--prod` bypasses + the two indicators (CLI stderr line in `_run`; idbar at `app.py:95,205`) + the `whoami` stage/hub enrichment (`cli.py:467-475`). Now every command inside a worktree auto-targets its stack and its `data_root`, `--prod` is the one-flag escape back to the global hub, and `whoami` answers "which hub, as whom". Kills surface 3 and the "evolve pointed at prod with dev-token" bug class.
