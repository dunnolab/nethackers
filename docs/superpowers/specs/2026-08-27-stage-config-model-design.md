...
1. **PR 1 — `config.py` + `--offline`.** `Stage` + `load_stage()` (defaults + env layer only; no file reading yet) + `DEV_TOKEN`/`DEV_OWNER`. Repoints every literal named in the entrypoint table: `cli.py:230` (deleting `_default_hub`, `:108-109`), `:269,281,308,321,331,405,414,544-545`; `launch.py:51-52,62-66,71,83,185`; `home.py:34`; `runs.py:21`; `evolve_form.py:48,326-327`; `register.py:33-34,62`. Fixes the stale help (`cli.py:197-198`). Golden test pins `Stage()` == today's literals (the config repoint is zero-behavior-change). Also introduces the explicit **`--offline`** flag (`EvolveParams.offline`) replacing the `owner=="dev"` publish sentinel at `launch.py:94` — one deliberate small behavior change, covered by its own publish-gate test. Kills K1, K3 (client), K5, K6, K7 **as drift** immediately.
