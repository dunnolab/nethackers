...
- [ ] If `arena/`, `nle-base/`, `uv.lock`, `src/nethackers/arena/` or
      ...
      needs a re-pin: run `.github/workflows/sandbox-images.yml` on the
      branch, then classify the new digest in
      `src/nethackers/arena_version.py`. A rebuild that cannot move a score
      goes under the current `ARENA_MAJOR`; one that can bumps it, which
      retires the verified corpus from every board until it is re-measured
      (nothing is deleted; [verifier.md](verifier.md)). A version bump
      touches `uv.lock`, so a routine bump is not routine here.
