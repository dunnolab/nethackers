...
ARENA_MAJOR_BY_DIGEST: dict[str, int] = {
    ...
    # v0.36.0 (the evolve UX rework), rebuilt for the same reason as the three
    # digests above: uv.lock is an arena input and the release moves the
    # project's own version in it, 0.35.0 -> 0.36.0. That line is the whole
    # uv.lock change (`git diff v0.35.0...HEAD -- uv.lock` is one hunk, the
    # `nethackers` package's own version), so every dependency version is the
    # one the previous arena installed. The release's code changes are all
    # host-side -- tui/ (seven modules), cli.py, diagnostics.py,
    # hubclient/publish.py and harness/launch.py -- and the arena's entrypoint
    # loads none of them (checked again on this build: importing
    # `nethackers.arena.run` leaves sys.modules holding only
    # nethackers.arena.* and nethackers.contracts.*). arena/ and contracts/
    # are untouched (`git diff v0.35.0...HEAD -- src/nethackers/arena
    # src/nethackers/contracts` is empty), and so are nle-base/Dockerfile and
    # arena/Dockerfile. Also the linux/amd64 LEG, not an index. Read from the
    # diff, NOT measured: no scoring-path code changed, so a re-run has
    # nothing to disconfirm. As on every rebuild, the base layer is whatever
    # `python:3.11-slim` resolves to today, which is why the digest moves at
    # all; the pinned dependency set on top of it does not.
    "sha256:273f9962cb31956c5632a6880fce1e47a2c828e56a39c261330132f7a8460c86": 2,
    # v0.36.1 (the tie fixes), rebuilt for the same reason as the digests above:
    # uv.lock is an arena input and the release moves the project's own version
    # in it, 0.36.0 -> 0.36.1. That one line is the whole uv.lock change, so
    # every dependency version is the one the previous arena installed. The
    # release's code changes are five host-side modules -- harness/archive.py,
    # harness/brief.py and three under tui/ -- and the arena's entrypoint loads
    # none of them (importing `nethackers.arena.run` leaves sys.modules holding
    # only nethackers.arena.* and nethackers.contracts.*). arena/ and contracts/
    # are untouched (`git diff v0.36.0..HEAD -- src/nethackers/arena
    # src/nethackers/contracts` is empty), and so are nle-base/Dockerfile and
    # arena/Dockerfile. Also the linux/amd64 LEG, not an index. Read from the
    # diff, NOT measured: no scoring-path code changed, so a re-run has nothing
    # to disconfirm. As on every rebuild, the base layer is whatever
    # `python:3.11-slim` resolves to today, which is why the digest moves at
    # all; the pinned dependency set on top of it does not.
    ...
    # v0.37.3 (the whole-/sol-tree readable fix), rebuilt because uv.lock is an
    # arena input and the release moves the project's own version, 0.37.2 ->
    # 0.37.3; not one package version moves, so the installed environment is
    # identical. The only non-lock change since the digest above is host-side
    # and never loaded by an episode -- eval/runner.py's recursive chmod of the
    # /sol mount (which makes THIS arena runnable under a umask-077 clone).
    # arena/ and contracts/ are untouched (`git diff v0.37.2...HEAD --
    # src/nethackers/arena src/nethackers/contracts` is empty), and so are
    # nle-base/Dockerfile and arena/Dockerfile. Also the linux/amd64 LEG, not an
    # index (I7). Read from the diff, NOT measured: no scoring-path code changed.
    # The digest moves only because uv.lock's self-version and the resolved
    # python:3.11-slim base layer differ.
    "sha256:0d0b0e779ebda4a05b5a22b39cfafcd2d2d9ef739ea60d9aae79052d55c767ae": 2,
}
