...
image + current seed list) so a Delta is never computed across epochs."""
...
def _read(store):
    return read_verified_baseline(
        store, secret_fingerprint=FP, seeds=SEEDS, evaluator_image=IMG
    )
...
def test_aggregates_progression_and_deepest_milestone_per_identity(tmp_path):
    ...
    _insert(s, [_atom(4839201, prog=0.4, milestone="Dlvl:3"),
                _atom(1029384, prog=0.6, milestone="Dlvl:5")])
    got = _read(s)
...
def test_overall_is_the_mean_across_identities(tmp_path):
    ...
    _insert(s, [_atom(4839201, identity="val-dwa-law-fem", prog=0.4),
                _atom(4839201, identity="wiz-elf-cha-mal", prog=0.2)])
    assert _read(s)["overall"] == 0.3


def test_excludes_other_epochs(tmp_path):
    """A floor measured under a rotated secret, a re-pinned arena, or a seed
    that is no longer in the hidden list must not leak into today's number --
    each would make Delta-vs-AA a comparison across different measurements."""
    s = _store(tmp_path)
    _insert(s, [_atom(4839201, prog=0.4)])
    _insert(s, [_atom(1029384, prog=1.0)], fp="c" * 64)          # rotated secret
    _insert(s, [_atom(1029384, prog=1.0, image="img@sha256:old")])  # re-pinned arena
    _insert(s, [_atom(999, prog=1.0)])                            # retired seed
    got = _read(s)
