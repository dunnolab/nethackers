...
def test_insert_verified_atoms_dedups_on_unique_key(tmp_path):
    ...
    assert (
        s.insert_verified_atoms(atoms, secret_fingerprint=FP,
                               verifier_token_fingerprint=TOKFP, arena_major=1) == 2
    )
    assert (
        s.insert_verified_atoms(atoms, secret_fingerprint=FP,
                               verifier_token_fingerprint=TOKFP, arena_major=1) == 0
    )
...
def test_rotation_new_secret_fingerprint_does_not_collide(tmp_path):
    s = _store(tmp_path)
    s.insert_verified_atoms([_atom(seed=1)], secret_fingerprint=FP,
                           verifier_token_fingerprint=TOKFP, arena_major=1)
    s.insert_verified_atoms([_atom(seed=1)], secret_fingerprint="c" * 64,
                           verifier_token_fingerprint=TOKFP, arena_major=1)
...
def test_iter_verified_atoms_filters(tmp_path):
    ...
    s.insert_verified_atoms([_atom(seed=1, identity="val-dwa-law-fem"),
                            ...
                            verifier_token_fingerprint=TOKFP, arena_major=1)
    got = s.iter_verified_atoms(secret_fingerprint=FP, evaluator_image=IMG,
...
def test_verified_attempt_latest_wins(tmp_path):
    s = _store(tmp_path)
    s.insert_verified_attempt(solution_digest="sha256:s", secret_fingerprint=FP,
                              evaluator_image=IMG, arena_major=1,
                              verifier_token_fingerprint=TOKFP,
                              status="failed", failure_kind="build_failed",
                              message="boom", identities_done=0, at="t1")
    s.insert_verified_attempt(solution_digest="sha256:s", secret_fingerprint=FP,
                              evaluator_image=IMG, arena_major=1,
                              verifier_token_fingerprint=TOKFP,
                              status="succeeded", failure_kind=None, message=None,
                              identities_done=73, at="t2")
    latest = s.latest_verified_attempt("sha256:s", secret_fingerprint=FP,
                                      arena_major=1)
    assert latest["status"] == "succeeded" and latest["identities_done"] == 73
    assert s.latest_verified_attempt("sha256:none", secret_fingerprint=FP,
                                    arena_major=1) is None
