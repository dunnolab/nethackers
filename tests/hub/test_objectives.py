...
def test_identity_objectives_are_single_character():
    ...
    assert len(spec0.batch) == 8  # per_identity_size default
    assert set(spec0.characters()) == {IDENTITIES[0]}
    assert [seed for seed, _character in spec0.batch] == list(range(8))

    # Every identity, not just the sample -- cheap (73 x 8) and much
